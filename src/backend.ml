...
let rec mld_of_md md =
  ...
  let rec loop ?(fst_p_in_li=true) ?(is_in_list=false) list_indent l =
    ...
    | X x :: tl ->
      ...
      loop list_indent tl
    | Blockquote q :: tl ->
      Buffer.add_string b (quote ~indent:list_indent (mld_of_md q));
    ...
    | Img(alt, src, title) :: tl ->
      Printf.bprintf b "{%%html: <img src=\"%s\"%s>%s</img>%%}"
        src (if alt = "" then "" else Printf.sprintf " alt=\"%s\"" alt) title;
      loop list_indent tl
    | Text t :: tl ->
    ...
    | Html(tagname, attrs, body) :: tl ->
      ...
      if a == attrs then
        ...
        Buffer.add_string b (Omd_backend.html_of_md body);
      Printf.bprintf b "</%s>" tagname;
      Buffer.add_string b "}\n";
      loop list_indent tl
    ...
    | Url (href,s,_title) :: tl ->
      Printf.bprintf b "{{: %s} %s}" href (mld_of_md s);
      loop list_indent tl
    | H1 md :: tl ->
    ...
    | H3 md :: tl ->
      ...
      loop list_indent tl
    | H4 md :: tl ->
      Buffer.add_string b "{3 ";
      loop list_indent md;
      Buffer.add_string b "}\n";
      loop list_indent tl
    | H5 md :: tl ->
      Buffer.add_string b "{4 ";
      loop list_indent md;
      Buffer.add_string b "}\n";
      loop list_indent tl
    | H6 md :: tl ->
      Buffer.add_string b "{5 ";
      loop list_indent md;
      Buffer.add_string b "}\n";
      loop list_indent tl
    | NL :: tl ->
