...
let string_of_attrs attrs =
  ...
  List.iter
    (function
      | a, Some v ->
        if not (String.contains v '\'')
        then Printf.bprintf b " %s='%s'" a v
        else if not (String.contains v '"')
        then Printf.bprintf b " %s=\"%s\"" a v
...
let rec mld_of_md ~min_header md =
  ...
  let rec loop ?(fst_p_in_li = true) ?(is_in_list = false) list_indent l =
    ...
    | Raw_block s :: tl ->
      ...
      Buffer.add_char b '\n';
      loop list_indent tl
    | Html (tagname, attrs, []) :: tl when StringSet.mem tagname html_void_elements ->
    ...
    | Html_block (tagname, attrs, body) :: tl ->
      let needs_newlines =
        ...
        | ( H1 _ | H2 _ | H3 _ | H4 _ | H5 _ | H6 _ | Ul _ | Ol _ | Ulp _ | Olp _
          ...
          | Img (_, _, _) )
          :: _ -> true
        | (Html_block (_, _, _) | Html_comment _ | Raw _ | Raw_block _) :: _ -> false
        | X _ :: _ -> false
        | NL :: _ -> false
        | [] -> false
      in
